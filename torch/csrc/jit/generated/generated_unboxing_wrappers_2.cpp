...
static auto registry = Registerer()
  // Generated operators
    ...
    .op("aten::_test_optional_intlist(Tensor values, int[]? addends) -> Tensor",
      [](OperatorKernel* unboxedKernel, const OperatorHandle&, Stack* stack) {
          using namespace at;
      
          auto result_ = callUnboxedKernel<Tensor, const Tensor &, c10::optional<IntArrayRef>>(unboxedKernel, (std::move(peek(*stack, 0, 2))).toTensor(),
          (std::move(peek(*stack, 1, 2))).toOptionalIntArray());
          drop(*stack, 2);
          pack(*stack, std::move(result_));
      })
