...
variable_list AngleBackward::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (should_compute_output({ self_ix })) {
    auto grad_result = any_grad_defined ? (grad.to(self_scalar_type) * (self*Scalar(c10::complex<double>{0.0, 1.0})).conj() / self.abs().pow(2)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CountNonzeroBackward0::apply(variable_list&& grads) {
  ...
  variable_list grad_inputs(gen.size());
  if (should_compute_output({ self_ix })) {
    auto grad_result = not_implemented("count_nonzero");
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list CountNonzeroBackward1::apply(variable_list&& grads) {
...
variable_list RepeatBackward::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (should_compute_output({ self_ix })) {
    auto grad_result = any_grad_defined ? (repeat_backward(grad, self_dim, repeats)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
